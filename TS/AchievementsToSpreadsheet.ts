...
function checkAndPostCertificationCourses(achievement: Achievement_Webhook) {
  ...
  else return "This achievement was not for certification.";
}

function postAchievementToSSv2(achievement: Achievement_Webhook, achievementType: number) {
...
function postAchievementToSS(achievement: Achievement_Webhook, achievementType: number) {
  var achievementSheetID, sheetName;
  //Configure where to post the new row to.
  switch(achievementType){
    case 1: {
  ...
  if (sheet){
    ...
    try {
      ...
      var thisAch = allAchvs.filter(ach => ach.CourseId==achievement.courseId);
